...
In the last two years I got passionate about the Sudoku puzzle, so I decided to write a Sudoku Solver and a Sudoku Generator that creates Sudoku with a unique solution (with the difficulties easy, medium and hard).<br>
...
The generator I've implemented creates a solved grid and then it adds <i>n</i> blank cells to make it "playable" (where <i>n</i> is related to the desired difficulty) while preserving the uniqueness of the grid's solution.
